...
function readDB() { try { return JSON.parse(fs.readFileSync(DB_FILE, 'utf8')); } catch (e) { return { users: [], messages: [], friendRequests: [] }; } }
function writeDB(data) { try { fs.writeFileSync(DB_FILE, JSON.stringify(data, null, 2)); } catch (e) { console.error(e); } }
initDatabase();
...
const sessionMiddleware = session({
    secret: 'cherry-blossom-secret-key-pro-v9',
...
// ==========================================
// --- 4. HỆ THỐNG SOCKET (CHAT + GAME) ---
// ==========================================
let ffaGameState = { players: {}, bullets: [] };
let zombieGameState = { players: {}, bullets: [], zombies: [], wave: 1 };
let pkRooms = {};
...
io.on('connection', (socket) => {
    ...
    socket.on('changePasswordInternal', ({ oldPass, newPass }, cb) => {
        if(checkBanStatus()) return; const db = readDB(); const user = db.users.find(u => u.id === currentUserId);
        if (user && bcrypt.compareSync(oldPass, user.password)) { user.password = bcrypt.hashSync(newPass, 10); writeDB(db); cb({ success: true, msg: "Đổi pass thành công!" }); } 
        else cb({ success: false, msg: "Pass cũ sai!" });
    });
    ...
    socket.on('adminUpdateUser', (data) => {
        const db = readDB(); if (db.users.find(u => u.id === currentUserId && u.role === 'admin')) {
            ...
            if (targetUser) {
                targetUser.displayName = data.displayName; writeDB(db); socket.emit('actionSuccess', "Đổi tên thành công!");
    ...
    // =====================================
    // GAME MODULES XỬ LÝ (FFA, ZOMBIE, PK)
    // =====================================

    // CHUNG: Input Xử lý chung
    socket.on('gameInput', ({ mode, roomId, keys, aimAngle, isShooting }) => {
        ...
        if(room && (mode !== 'pk' || room.status === 'playing')) {
            ...
            if(p && p.hp > 0) {
                p.keys = keys;
                if(isShooting && (!p.lastShot || Date.now() - p.lastShot > 150)) { // Bắn mỗi 150ms
                    p.lastShot = Date.now();
                    room.bullets.push({ 
    ...
    socket.on('joinFFAGame', () => { 
        if(checkBanStatus()) return; const db = readDB(); const user = db.users.find(u => u.id === currentUserId); if(!user) return; 
        socket.join('ffaGameRoom'); 
        ffaGameState.players[socket.id] = { id: socket.id, userId: currentUserId, name: user.displayName || user.username, x: Math.random() * 600 + 100, y: Math.random() * 400 + 50, color: `hsl(${Math.random() * 360}, 100%, 70%)`, hp: 100, score: user.gameScore || 0, keys: { w: false, a: false, s: false, d: false } }; 
    });
    socket.on('leaveFFAGame', () => { socket.leave('ffaGameRoom'); delete ffaGameState.players[socket.id]; });

    // Zombie Mode
    socket.on('joinZombieGame', () => {
        if(checkBanStatus()) return; const db = readDB(); const user = db.users.find(u => u.id === currentUserId); if(!user) return; 
        socket.join('zombieGameRoom'); 
        zombieGameState.players[socket.id] = { id: socket.id, userId: currentUserId, name: user.displayName || user.username, x: Math.random() * 600 + 100, y: Math.random() * 400 + 50, color: '#3b82f6', hp: 100, score: 0, keys: { w: false, a: false, s: false, d: false } }; 
    });
    socket.on('leaveZombieGame', () => { socket.leave('zombieGameRoom'); delete zombieGameState.players[socket.id]; });
    ...
    socket.on('acceptPK', (inviterId) => {
        ...
        pkRooms[roomId] = { id: roomId, status: 'waiting', players: { [inviterSocketId]: { id: inviterSocketId, userId: p1.id, name: p1.displayName || p1.username, x: 200, y: 250, color: '#f87171', hp: 100, score: 0 }, [socket.id]: { id: socket.id, userId: p2.id, name: p2.displayName || p2.username, x: 600, y: 250, color: '#60a5fa', hp: 100, score: 0 } }, bullets: [] };
    ...
    socket.on('startPK', (roomId) => {
        if(pkRooms[roomId] && pkRooms[roomId].status === 'waiting' && Object.keys(pkRooms[roomId].players).length === 2) {
            ...
            for(let sid in pkRooms[roomId].players) { let p = pkRooms[roomId].players[sid]; p.hp = 100; p.score = 0; p.x = i === 0 ? 200 : 600; p.y = 250; i++; }
            pkRooms[roomId].bullets = []; io.to(roomId).emit('pkGameStarted', pkRooms[roomId]);
        }
    });
    socket.on('leavePK', (roomId) => { socket.leave(roomId); if(pkRooms[roomId]) { delete pkRooms[roomId].players[socket.id]; io.to(roomId).emit('actionError', "Đối phương đã thoát phòng PK."); io.to(roomId).emit('pkRoomClosed'); delete pkRooms[roomId]; } });
    ...
    socket.on('disconnect', () => { 
        ...
        delete zombieGameState.players[socket.id];
...
// =====================================
// ENGINE GAME TỔNG HỢP CHẠY 60FPS
// =====================================
setInterval(() => {
    const handleMovement = (players, speed = 5) => {
    ...
    // Quái vật sinh sản
    if(Object.keys(zombieGameState.players).length > 0 && zombieGameState.zombies.length < 30 && Math.random() < 0.05) {
        zombieGameState.zombies.push({ x: Math.random() > 0.5 ? -30 : 830, y: Math.random() * 500, hp: 40, id: Math.random() });
    }
    
    // Quái vật AI di chuyển
    zombieGameState.zombies.forEach(z => {
        let target = null, minDist = Infinity;
        for(let id in zombieGameState.players) {
            let p = zombieGameState.players[id];
            if(p.hp > 0) { let d = Math.hypot(p.x - z.x, p.y - z.y); if(d < minDist) { minDist = d; target = p; } }
        }
        if(target) {
            let angle = Math.atan2(target.y - z.y, target.x - z.x);
            z.x += Math.cos(angle) * 1.5; z.y += Math.sin(angle) * 1.5;
            if(minDist < 25) target.hp -= 1; // Cắn mất máu từ từ
        }
    });
    ...
    zombieGameState.bullets.forEach(b => {
        zombieGameState.zombies.forEach(z => {
            if(b.life > 0 && Math.hypot(z.x - b.x, z.y - b.y) < 20) {
                z.hp -= 20; b.life = 0;
                if(z.hp <= 0 && zombieGameState.players[b.ownerId]) zombieGameState.players[b.ownerId].score += 5;
            }
        });
    });
    zombieGameState.zombies = zombieGameState.zombies.filter(z => z.hp > 0);
    
    // Hồi sinh nếu chết trong Zombie mode
    for(let id in zombieGameState.players) {
        let p = zombieGameState.players[id];
        if(p.hp <= 0 && !p.respawning) {
            p.respawning = true;
            setTimeout(() => { if(zombieGameState.players[id]) { zombieGameState.players[id].hp = 100; zombieGameState.players[id].respawning = false; zombieGameState.players[id].x = 400; zombieGameState.players[id].y = 250; } }, 4000);
        }
    }
                    ...
                    if(p.hp > 0 && b.ownerId !== sid && Math.hypot(p.x - b.x, p.y - b.y) < 22) {
                        ...
                        if(p.hp <= 0 && room.players[b.ownerId]) {
                            ...
                            if(room.players[b.ownerId].score >= 100) {
                                room.status = 'waiting'; io.to(roomId).emit('pkGameOver', { winnerName: room.players[b.ownerId].name });
                            } else {
...
server.listen(PORT, () => console.log(`Cherry Server V9 MAX running on port ${PORT}`));